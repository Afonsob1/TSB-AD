...
class CNN_RW():
    ...
    def fit(self, data, train_idx= 1000):
        ...
        {
        'params': [ts_changes],
        'lr': self.lr*10 # higher learning rate for go down to 0 faster
        }])
        ...
        scores = np.abs(scores - np.mean(scores[:train_idx], axis=1, keepdims=True)) / (np.std(scores[:train_idx], axis=1, keepdims=True) + 1e-8)
        scores = scores.mean(axis=0)

        savgol_filter_scores = savgol_filter(scores, self.window_size, 1)

        scores = scores.clip(np.mean(scores) + 1*np.std(scores), np.max(scores)) 

        savgol_filter_scores = savgol_filter_scores + scores

        return savgol_filter_scores

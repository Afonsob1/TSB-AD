...
for ((i=0; i<num_datasets; i++)); do
  ...
  if [ -f "$dataset" ]; then
    ...
    CUDA_VISIBLE_DEVICES="$gpu_index" python -m benchmark_exp.Run_Detector_M --AD_Name PCA --save True --dataset_dir "$dataset_dir" --filename "${filename_without_ext}.csv" >> "$output_file" &
